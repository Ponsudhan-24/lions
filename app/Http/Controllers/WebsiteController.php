...
public function donationForm()
{
  ...
  public function showCareerEnquiryForm() {
    return view('member.careerenquiry');
}
...
{
    $type = $request->query('type');              // e.g. 'bottom'
    $url = $request->query('url');                // target redirect
    $image = $request->query('image');            // image_path

    if (!$url || !$image) {
        abort(400, 'Invalid click tracking request.');
    }

    $click = BannerClick::firstOrCreate(
        ['banner_type' => $type, 'image_path' => $image, 'redirect_url' => $url],
        ['click_count' => 0]
    );

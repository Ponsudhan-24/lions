...
class MemberLoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (!$member) {
            Log::warning('Login failed: Member not found for member_id: ' . $request->member_id);
            return back()->with('error', 'Member ID not found.');
        }
    
        Log::info('Member found: ' . $member->member_id);
    
        if (!Hash::check($request->password, $member->password)) {
            Log::warning('Login failed: Password mismatch for member_id: ' . $request->member_id);
